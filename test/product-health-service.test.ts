...
describe("ProductHealthService", () => {
  beforeEach(async () => {
    ...
    await env.DB.exec("DELETE FROM notification_events; DELETE FROM regional_product_health; DELETE FROM regional_products; DELETE FROM games;");
    await env.DB.batch([
  ...
  it("persists the third-failure alert state and emits one recovery after a later success", async () => {
    ...
    await expect(env.DB.prepare("SELECT event_type AS eventType, status FROM notification_events ORDER BY created_at").all<{ eventType: string; status: string }>()).resolves.toMatchObject({ results: [{ eventType: "collection-failure", status: "pending" }, { eventType: "collection-recovered", status: "pending" }] });
  });
});
