...
import gameNameSourcesSchema from "../migrations/0007_game_name_sources.sql?raw";

beforeAll(async () => {
  // Cloudflare 测试 D1 对整段多语句 exec 的兼容性有限；逐条 prepare/run 更接近生产迁移的原子语句执行方式。
  // 测试必须按生产编号顺序执行所有迁移；0007 为既有游戏补来源默认值，遗漏会使新旧名称来源断言在不完整 schema 上产生假结果。
  for (const statement of [...splitStatements(coreSchema), ...splitStatements(priceTrackingSchema), ...splitStatements(authSchema), ...splitStatements(manualRefreshSchema), ...splitStatements(subscriptionConfirmationSchema), ...splitStatements(immediateManualRefreshSchema), ...splitStatements(gameNameSourcesSchema)]) {
