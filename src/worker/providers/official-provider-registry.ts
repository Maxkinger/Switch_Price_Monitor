...
export class OfficialProviderRegistry {
  private readonly japanesePriceApi: PriceProvider;

  public constructor(private readonly fetcher: typeof fetch = fetch) {
    // 日区 API 适配器会自行再次核验 JP、JPY 与本区价格 ID；在注册表和适配器两层约束可抵抗错误调用方。
    this.japanesePriceApi = createNintendoPriceApiProvider(fetcher);
  }

  /**
   * 按地区返回按优先级排序的官方来源。日区优先使用返回地区价格 ID 的公开 API，若不可用再尝试同一商品的官方 JSON-LD；
   * 其余四区不复用日区 API 规则，只使用本区商品链接的结构化公开数据，并由后续 ProviderChain 校验商品身份。
   */
  public providersFor(product: RegionalProduct): PriceProvider[] {
    ...
    return product.regionCode === "JP" ? [this.japanesePriceApi, officialPage] : [officialPage];
  }
}
