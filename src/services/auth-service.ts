...
import type { AuthRepository, StoredSession } from "../repositories/ports";
import {
 ...
 * 所有秘密只在调用边界短暂存在；仓储只接收派生哈希或令牌摘要，避免数据库副本直接成为登录凭据。
 */
const encoder = new TextEncoder();
// PBKDF2 通过标准 Web Crypto 在 Node 与浏览器兼容运行时可用；10 万次迭代提高离线破解成本，同时保持个人站点首次登录的可接受延迟。
...
/** 管理路由只依赖原始 Cookie 的认证判断，不需要密码、恢复或会话写入能力。 */
export interface SessionReader {
  authenticate(token: string, now: string): Promise<boolean>;
}

export class AuthService implements SessionReader {
  ...
  public async initialize(
    ...
    if (await this.repository.isInitialized()) throw new ConflictError("初始化已完成。");
    ...
    try {
      ...
      // PostgreSQL 唯一约束是并发初始化的最终裁决；只转换已分类冲突，其他存储故障由路由脱敏为通用 500。
      if (error instanceof AuthInitializationConflictError) throw new ConflictError("初始化已完成。");
      throw error;
    }
    return { recoveryCode };
  }
  ...
  public async login(password: string, now: string): Promise<{ token: string; expiresAt: string }> {
    ...
    if (!credential || !(await matches(password, credential.passwordSalt, credential.passwordHash))) {
      // 原子递增由仓储在单条 SQL 中完成；自然到期的旧锁会在该语句内从一重新累计，防止并发读改写丢失次数。
      await this.repository.recordFailedLogin({
        now,
        lockedUntil: new Date(nowMs + loginLockDurationMs).toISOString(),
        maximumFailedLogins,
      });
    ...
    // 条件会话事务会重新锁定并比较刚验证的哈希/盐，同时清失败状态；密码恢复若已提交，旧凭据不能在其后插入逃逸会话。
    const establishment = await this.repository.establishSession({ expectedCredential: credential, session, now });
    if (establishment === "locked") throw new LoginLockedError("登录尝试过多，请十五分钟后再试。");
    if (establishment === "credential-changed") throw new InvalidCredentialsError("密码错误。");
  ...
  public async resetPassword(recoveryCode: string, password: string, now: string): Promise<void> {
    ...
    const passwordSalt = randomText(16);
    try {
      // 仓储事务同时更新密码、消费恢复状态、撤销全部会话并清失败记录；任一步失败都不能留下半恢复账户。
      ...
      // 两个并发恢复即使都完成哈希验证，也只有一个条件事务能消费恢复状态；失败方沿用既有安全文案。
      if (error instanceof AuthRecoveryAlreadyUsedError) throw new InvalidRecoveryCodeError("恢复码无效或已使用。");
      throw error;
    }
  }
...
/** 哈希输出统一为十六进制文本，便于数据库 TEXT 字段保存和参数化查询，并保持迁移前格式完全一致。 */
