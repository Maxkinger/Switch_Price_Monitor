import type { ManualRefreshRequestStore } from "../repositories/ports";
...
/**
 * 手动刷新服务先以仓储原子记录取得刷新名额，再在同一 HTTP 请求内运行统一采集器。
 * 它不直接解析任天堂或第三方页面，因此手动与 Cron 路径仍复用完全相同的来源、汇率、健康检查和通知规则。
 */
export class ManualRefreshService {
  public constructor(
    private readonly requests: ManualRefreshRequestStore,
