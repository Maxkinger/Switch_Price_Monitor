...
export async function handleSettingsRoute(request: Request, dependencies: SettingsRouteDependencies): Promise<Response | null> {
  ...
  try {
    ...
    if (isProxyTest) return Response.json(await dependencies.proxyTest!.test(readProxySettings(await request.json<unknown>())));
    if (request.method === "GET") return Response.json(await service.get());
    const result = await service.update(readPatch(await request.json<unknown>(), dependencies.proxySupported === true), (dependencies.now ?? defaultNow)());
