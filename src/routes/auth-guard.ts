 ...
 */
export async function requireAdmin(request: Request, sessions: SessionReader, now = new Date().toISOString()): Promise<boolean> {
  const token = readSessionCookie(request.headers.get("cookie"));
  return sessions.authenticate(token, now);
}
