...
/** 一份代理草稿只保存可公开管理的端点字段，用户名、密码和认证 URL 不属于该模型。 */
...
/** 默认关闭状态只作为可编辑草稿，首次升级不会据此创建任何代理连接。 */
export const defaultProxySettings: ProxySettings = { enabled: false, protocol: "http", host: "127.0.0.1", port: 7890 };

/**
 * 将主机规范为 URL 解析器认可的稳定形式，拒绝完整 URL、认证信息、路径和控制字符。
 * IPv6 临时加方括号只为解析；持久化值始终是裸主机，避免不同消费者拼接出不一致的代理地址。
 */
export function normalizeProxyHost(value: string): string | null {
  ...
  try {
    ...
    return hostname.startsWith("[") && hostname.endsWith("]") ? hostname.slice(1, -1) : hostname || null;
  } catch { return null; }
}

/** 即使代理关闭也验证全部草稿，防止未来启用时由旧无效值绕过服务端边界。 */
