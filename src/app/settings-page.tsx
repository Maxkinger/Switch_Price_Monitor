      ...
      <form className="settings-form" onSubmit={(event) => void handleSubmit(event)}>
        ...
        <fieldset className="settings-card">
          ...
          <label className="settings-choice">
            <input type="checkbox" checked={draft.proxy.enabled} onChange={(event) => setDraft((current) => current ? { ...current, proxy: { ...current.proxy, enabled: event.target.checked } } : current)} />
            启用网络代理
          </label>
...
/** 将服务端三态映射为固定中文文案；不向页面暴露代理主机、URL、HTTP 响应正文或底层异常。 */
