...
import { dashboardPath, gameNameManagementPath, readAppRoute, settingsPath, subscriptionNewPath, type AppRoute } from "./app-navigation";
...
import { createGameNameApiClient } from "./game-name-api-client";
import { GameNameManagementPage } from "./game-name-management-page";
...
export function AppShell({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  // 四类客户端共用 tracker，但仍按领域拆分 API 面，避免任一页面取得不需要的写入方法或外站访问能力。
  ...
  const gameNameApi = useMemo(() => createGameNameApiClient(fetch, requestTracker), [requestTracker]);
  ...
  return (
    <div className="monitor-app">
      <aside className="monitor-nav" aria-label="主导航">
        ...
        <button type="button" className={route.kind === "game-names" ? "monitor-nav__active" : ""} onClick={() => navigate(gameNameManagementPath())}>中文名称</button>
        <span>价格历史（即将提供）</span>
      ...
      <main className="monitor-main">
        ...
        {route.kind === "subscription-new" ? <SubscriptionWizardPage api={productApi} gameNameApi={gameNameApi} onUnauthorized={onUnauthorized} /> : null}
        {route.kind === "subscription-detail" ? <SubscriptionDetailPage api={dashboardApi} gameNameApi={gameNameApi} productApi={productApi} subscriptionId={route.subscriptionId} onBack={() => navigate(dashboardPath())} onUnauthorized={onUnauthorized} /> : null}
        {route.kind === "game-names" ? <GameNameManagementPage api={gameNameApi} onUnauthorized={onUnauthorized} /> : null}
