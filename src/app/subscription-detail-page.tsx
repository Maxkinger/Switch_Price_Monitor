...
export function SubscriptionDetailPage({ api, subscriptionId, onBack, onUnauthorized }: { api: DetailApi; subscriptionId: string; onBack: () => void; onUnauthorized: () => void }) {
  ...
  async function resolveMissingRegions(): Promise<void> {
    ...
    try {
      ...
      setMissingResolutions(resolutions); setMissingConfirmations({}); setMissingSkipped([]);
    } catch (error) {
      if (error instanceof DashboardApiError && error.status === 401) onUnauthorized();
  ...
  return <section className="detail-page" aria-labelledby="detail-title">
    ...
    <section className="detail-management"><h2>管理订阅</h2><fieldset><legend>补全已启用地区</legend><button className="secondary-button" type="button" disabled={isResolvingMissing} onClick={() => void resolveMissingRegions()}>{isResolvingMissing ? "解析中…" : "补全已启用地区"}</button>{missingResolutions.map((resolution) => <div key={resolution.regionCode}><b>{resolution.regionCode}</b>：{resolution.status === "automatic" ? <button type="button" className="text-button" onClick={() => acceptMissingCandidate(resolution, resolution.candidate, "automatic")}>采用自动匹配</button> : null}{resolution.status === "needs-manual-selection" ? resolution.candidates.map((candidate) => <button type="button" className="text-button" key={candidate.productUrl} onClick={() => acceptMissingCandidate(resolution, candidate, "manual_selection")}>采用 {candidate.canonicalTitle}</button>) : null}{resolution.status === "needs-manual-link" ? <input aria-label={`${resolution.regionCode} 官方链接`} placeholder="粘贴任天堂官方商品链接后按 Enter" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void resolveMissingLink(resolution.regionCode, event.currentTarget.value); } }} /> : null}<button type="button" className="text-button" onClick={() => setMissingSkipped((current) => current.includes(resolution.regionCode) ? current.filter((region) => region !== resolution.regionCode) : [...current, resolution.regionCode])}>{missingSkipped.includes(resolution.regionCode) ? "取消跳过" : "跳过此区"}</button>{missingConfirmations[resolution.regionCode] ? <small> 已确认</small> : null}</div>)}{missingResolutions.length > 0 ? <button className="primary-button" type="button" disabled={isCompletingMissing || missingResolutions.some((resolution) => !missingConfirmations[resolution.regionCode] && !missingSkipped.includes(resolution.regionCode))} onClick={() => void completeMissingRegions()}>{isCompletingMissing ? "补全中…" : "确认补全"}</button> : null}</fieldset><fieldset><legend>监控地区</legend>{detail.regions.map((region) => <label key={region.regionalProductId}><input type="checkbox" checked={selectedIds.has(region.regionalProductId)} onChange={() => setSelectedIds((current) => { const next = new Set(current); if (next.has(region.regionalProductId)) next.delete(region.regionalProductId); else next.add(region.regionalProductId); return next; })} />{region.regionCode}（已确认商品）</label>)}<button className="secondary-button" type="button" disabled={selectedIds.size === 0 || [...selectedIds].every((id) => monitoredIds.has(id))} onClick={() => void save({ regionalProductIds: [...selectedIds] }, "监控地区已保存。")}>保存监控地区</button></fieldset>
