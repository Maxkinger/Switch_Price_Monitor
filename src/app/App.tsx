...
/**
 * 添加订阅向导。它只管理本次选择过程，最终写入前由后端重新验证每个官方链接并原子化创建订阅；
 * 因此刷新或取消页面不会留下部分游戏、地区商品或未经验证的价格记录。
 */
export function App() {
  ...
  async function handleSearch(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice(error instanceof ProductApiError ? error.message : "官方搜索暂时不可用，请稍后重试。");
    } finally {
  ...
  async function handleFallbackLink(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice(error instanceof ProductApiError ? error.message : "官方链接核验未完成，请稍后重试。");
    } finally {
      setIsSearching(false);
    }
  }
  ...
  async function handleResolveRegions() {
    ...
    try {
      ...
      setNotice(error instanceof ProductApiError ? error.message : "跨区匹配未完成，请稍后重试。");
    } finally {
  ...
  async function handleResolveRegionalLink(selected: OfficialProductCandidate, regionCode: RegionCode) {
    ...
    try {
      ...
      setNotice(error instanceof ProductApiError ? error.message : "地区商品链接核验失败，请检查链接后重试。");
    } finally {
  ...
  async function handlePreviewSources() {
    ...
    try {
      ...
      setNotice(error instanceof ProductApiError ? error.message : "来源预览未完成，请稍后重试。");
    }
  }
  ...
  async function handleConfirmSubscriptions() {
    ...
    try {
      ...
      setNotice(error instanceof ProductApiError ? error.message : "订阅确认未完成，请稍后重试。");
    }
  }
