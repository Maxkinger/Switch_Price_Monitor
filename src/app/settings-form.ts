...
/**
 * 从同源设置 API 的完整公开 DTO 建立浏览器草稿。地区数组必须复制，防止复选框更新意外修改先前响应对象；
...
export function createSettingsForm(settings: AppSettings): SettingsFormState {
  return {
    ...
    // 旧设置响应缺少代理字段时只在浏览器创建关闭草稿；真正启用仍需服务端保存完整四字段并再次校验。
 ...
 * Node 服务会在保存时再次验证，前端联动仅用于即时且可理解的交互反馈。
 */
 ...
 * 该函数不做服务端校验；页面提交后仍以服务端的地区、时区、时间和税务州规则为准。
 */
export function toPublicSettingsPatch(state: SettingsFormState): PublicSettingsPatch {
  return {
    ...
    // 逐字段复制，禁止把服务端未来加入的认证、URL 或内部字段随对象展开提交。
    proxy: { ...state.proxy },
  };
}
