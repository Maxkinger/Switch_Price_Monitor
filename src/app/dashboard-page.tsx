...
import { DashboardApiError, type DashboardOverview } from "./dashboard-api-client";
import { formatCnyFen, formatLocalPrice } from "./dashboard-view-model";

/** 仪表盘仅要求客户端具备的读取/排队能力，保持页面可在测试中注入受控端口。 */
interface DashboardPageApi {
  getDashboard(): Promise<DashboardOverview>;
  queueRefresh(): Promise<{ status: "queued"; requestedAt: string; nextAllowedAt: string }>;
}

/**
 * 概览优先首页。价格和状态完全来自 Worker；当前页面不请求任天堂、第三方站点、汇率服务或 Telegram，
 * 手动刷新也只进入服务端队列，防止浏览器把“已排队”误说成抓取已完成。
 */
export function DashboardPage({ api, onNavigate, onUnauthorized }: { api: DashboardPageApi; onNavigate: (path: string) => void; onUnauthorized: () => void }) {
  ...
  async function queueRefresh(): Promise<void> {
    setNotice(null);
    try { await api.queueRefresh(); setNotice("已排队，等待采集任务执行。"); }
    catch (error) {
      if (error instanceof DashboardApiError && error.status === 401) onUnauthorized();
      else setNotice(error instanceof DashboardApiError ? error.message : "刷新请求暂时无法提交。");
    }
  }
  ...
  return <section className="dashboard-page" aria-labelledby="dashboard-title">
    <header className="dashboard-header"><div><h1 id="dashboard-title">仪表盘</h1><p>查看当前价格与历史最低价。</p></div><div><button className="secondary-button" type="button" onClick={() => void queueRefresh()}>立即刷新</button><button className="primary-button" type="button" onClick={() => onNavigate(subscriptionNewPath())}>添加订阅</button></div></header>
