...
usage() {
  ...
  printf '%s\n' 'usage: restore-postgres.sh --compose-file ABSOLUTE --env-file ABSOLUTE --project-name NAME --app-service NAME --database-service NAME --project-root ABSOLUTE --backup-dir ABSOLUTE --dump ABSOLUTE --database NAME' >&2
}
...
compose_file=''; env_file=''; project_name=''; app_service=''; database_service=''; project_root=''; backup_dir=''; dump_file=''; target_database=''
while [[ $# -gt 0 ]]; do
  case "$1" in
    --compose-file|--env-file|--project-name|--app-service|--database-service|--project-root|--backup-dir|--dump|--database)
      require_value "$1" "${2-}"
      case "$1" in
        --compose-file) compose_file="$2" ;; --env-file) env_file="$2" ;; --project-name) project_name="$2" ;; --app-service) app_service="$2" ;;
...
[[ "$compose_file" == /* && "$env_file" == /* && "$project_root" == /* && "$backup_dir" == /* && "$dump_file" == /* ]] || die '路径必须为绝对路径'
...
[[ -f "$compose_file" && -f "$env_file" && -d "$project_root" && -d "$backup_dir" && -f "$dump_file" ]] || die '路径类型不符合要求'
...
# 所有恢复子命令固定使用显式绝对 env 文件；DSM 任意工作目录不能影响变量解析，也不能回退读取其他项目 `.env`。
# 函数只注入已校验的 Compose 前缀且从不打印 config 展开结果，避免数据库和 Telegram 秘密进入任务日志。
...
trap cleanup EXIT
compose config --services 2>/dev/null | grep -Fx -- "$app_service" >/dev/null || die '应用服务不存在'
compose config --services 2>/dev/null | grep -Fx -- "$database_service" >/dev/null || die '数据库服务不存在'
...
assert_app_stopped() {
  ...
  all_ids="$(compose ps --all -q "$app_service" 2>/dev/null)"
  safe_ids="$(compose ps --all --status exited -q "$app_service" 2>/dev/null; compose ps --all --status dead -q "$app_service" 2>/dev/null)"
...
manifest="$(compose run --rm --no-deps --entrypoint sh "$app_service" -ceu 'for f in /app/migrations/postgres/*.sql; do test -f "$f" || exit 1; line="$(sha256sum "$f")"; checksum="${line%% *}"; printf "%s|%s\\n" "${f##*/}" "$checksum"; done | LC_ALL=C sort' 2>/dev/null)" || die '迁移校验失败'
...
read_target_state() {
  compose exec -T "$database_service" sh -ceu '
...
if ! cat -- "$dump_file" | compose exec -T "$database_service" sh -ceu '
...
# 此处是迁移验证模式：恢复后必须有非空不可变迁移账本，且 public 表集合与当前唯一初始迁移精确一致。
...
[[ "$validation" =~ ^[1-9][0-9]*:16:16:0[[:space:]]*$ ]] || fail_post_validation '恢复后的迁移或完整表集合验证失败'
