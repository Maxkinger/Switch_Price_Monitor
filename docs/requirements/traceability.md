...
| ID | 需求主题 | 状态 | 对应设计/决策 |
| --- | --- | --- | --- |
| FR-001 | 多商品订阅、默认区、跨区匹配与手动修正 | 已确认 / 已实施部分（日区升级包自动关系发现尚未准入生产；新增与补全流程均由服务端保存的 `enabledRegions` 决定范围；US、MX、BR 使用各自任天堂官方索引，HK 与 JP 使用各自官方公开搜索入口。唯一严格身份匹配仅以 `automatic` 自动加入；同类型的语言化或歧义官方候选须由管理员选择。日区 `DL_DLC` 与港区官方 `bundles` 已纳入美食家版受控等价匹配。日区软件搜索只提供完整版的 `upgrade: 1`，普通 Worker 请求详情只得到缺少关系链接的 JavaScript 排队页，因此现阶段仅批准独立 Browser Run 探针对已知样本执行三次只读验证；三次均须在 30 秒内唯一返回同一升级包官方 URL，且不复用会话。全通过只进入生产设计，任一次失败则继续人工官方链接。港区只请求普通官网一次，首轮无同类型候选时对已审核后缀执行一次基础标题搜索，限制 1–5 个 `titles` 根和每根 50 个一层官方关系，再复核关联商品自己的详情；不再依赖从 Worker 返回 403 的 Magento 搜索。任一关系不完整会回退人工链接，非日区 automatic 在最终写入前还会复用同一发现服务重新证明 URL 唯一，失效时零写入；日区普通商品继续使用搜索与价格双官方 API。自动与人工来源分别校验，并以单个 D1 批次原子写入） | PRD §3；系统设计 §3.1；API 设计 §4；官方价格 ID 设计规格；官方订阅发现与批量确认设计；设置驱动的地区补全设计；多地区任天堂官方搜索与自动监控设计；跨语言地区商品高置信度识别设计；日区订阅确认官方 API 复核设计；日区升级包官方关系发现设计；美食家版跨区官方组合商品识别设计；香港官方关联商品发现实施计划 |
