...
| ID | 需求主题 | 状态 | 对应设计/决策 |
| --- | --- | --- | --- |
| FR-001 | 多商品订阅、默认区、跨区匹配与手动修正 | 已确认 / 已实施（新增与补全流程均由服务端保存的 `enabledRegions` 决定范围；US、MX、BR 使用各自任天堂官方索引，HK 与 JP 使用各自官方公开搜索入口。唯一严格身份匹配仅以 `automatic` 自动加入；同类型的语言化或歧义官方候选须由管理员选择，搜索为空或不可用才允许官方链接/跳过。完整标题不同但拉丁主标题、受控版本标记、发行商、商品类型都一致且唯一的本地化商品，现可由 Worker 高置信度自动识别；证据不足仍须人工选择，并由服务端排序与 `featuredCandidateCount` 决定候选首屏。最终确认重新解析本区官方链接，自动与人工来源分别校验，并以单个 D1 批次原子写入。固定夹具与本地质量门禁已通过；新规则尚待部署和生产只读验收） | PRD §3；系统设计 §3.1；API 设计 §4；官方价格 ID 设计规格；官方订阅发现与批量确认设计；设置驱动的地区补全设计；多地区任天堂官方搜索与自动监控设计；跨语言地区商品高置信度识别设计 |
