...
| `notification_events` | 通知去重与审计 | 类型、关联订阅/地区、状态变迁、Telegram 发送结果和时间；唯一键原子预留同一业务事件的一次发送资格，成功投递时间只可由 pending 安全更新一次；投递调度按创建顺序仅读取 pending，已 delivered 的事件不会再次进入发送队列 |
