...
| `games` | 逻辑商品主档 | 简体中文展示名、展示名来源、原官方标题、规范化标题、发行商、商品类型、封面、唯一规范化标识；人工名称不得被普通同步覆盖 |
...
- `migrations/0007_game_name_sources.sql` 为既有 `games` 增加非空 `name_zh_source`，默认 `legacy_pending_sync` 且不改写既有 `name_zh`、不发起外部请求。`games.name_zh` 是仪表盘、详情、日报和通知共用的展示名；其来源为大陆官方、香港官方、人工中文、官方英文回退或历史待同步。大陆官方名称只接受经唯一香港 `titles/{数字 ID}` 核验后同 ID 的腾讯 Nintendo Switch `software/{同一数字 ID}` 标题；没有大陆页面时才离线把已核验的香港繁中标题转为简体。两种名称处理均只改变显示文字，不参与商品身份计算。普通同步跳过 `manual_chinese`；管理员提交最终回退决定时会再次解析官方名称，官方成功始终优先。
