...
| `POST /api/products/preview-game-names` | 已登录管理员 | 接受与最终确认同形且在路由边界收窄的商品候选和地区映射，只预览大陆官方、香港官方或不可用状态，不写入 D1。Worker 从受控锚点与已确认的香港 URL 再次解析，不信任浏览器声明的名称来源、标题或外部内容；匿名请求在读取请求体或访问任天堂前返回 `401`。 |
