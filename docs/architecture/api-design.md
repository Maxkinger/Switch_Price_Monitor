...
| `PATCH /api/subscriptions/:id` | 已登录管理员 | 当前仅接受 `{ "enabled": true }` 重新启用已停用订阅，返回更新后的状态；地区和目标价编辑将在后续接口迭代中开放，未知字段不会被静默保存。 |
...
| `GET /api/export?kind=prices` | 已登录管理员 | 下载价格历史 CSV；字段白名单仅含地区、金额、币种、人民币金额、来源和采集时间，不包含认证、会话、恢复码或 Telegram 字段。 |
