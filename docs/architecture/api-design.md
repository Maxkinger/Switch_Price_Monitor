...
| `POST /api/products/confirm-subscriptions` | 已登录管理员 | 接受非空商品数组；每项必须有默认区官方候选、至少一个不重复地区映射及受控匹配来源。Worker 在写入前重新解析每个官方链接并二次验证本区官方价格 ID，再以单个 D1 批次创建所有新游戏、地区商品、订阅和关联。任一项无效返回 `422` 且不写入；同一规范化游戏已有订阅时按输入顺序返回 `existing`，不改写既有地区范围。包含任一新建项返回 `201`，全为既有订阅返回 `200`。 |
