...
| 订阅管理 | 创建、读取、编辑、软停用、重新启用、永久删除、目标价、地区配置和游戏名称同步 |
...
| `POST /api/products/preview-game-names` | 已登录管理员 | 接受与最终确认同形且在路由边界收窄的商品候选和地区映射，只预览大陆官方、香港官方或不可用状态，不写入 D1。向导在“核验其他地区”成功及人工 HK 官方链接成功后调用它，以 Worker 已核验的候选展示中文主标题与官方原标题副标题；这不是保存授权，最终确认仍会重新核验。Worker 从受控锚点与已确认的香港 URL 再次解析，不信任浏览器声明的名称来源、标题或外部内容；匿名请求在读取请求体或访问任天堂前返回 `401`。 |
...
| `POST /api/subscriptions/sync-game-names` | 已登录管理员 | 仅处理请求中明确列出的既有订阅。Worker 从受订阅归属约束的保存数据重建香港官方 `titles/{数字 ID}`，优先使用同 ID 腾讯 Nintendo Switch `software/{同一数字 ID}` 大陆标题；没有该页面时才离线转换香港繁中标题。普通同步不覆盖 `manual_chinese`，无法解析时返回逐项官方英文锚点和人工决定要求；不回显外部正文、Cookie、内部评分或秘密。 |
| `POST /api/subscriptions/sync-game-names/confirm` | 已登录管理员 | 仅接受去重订阅 ID 与可选的 1–200 字含汉字人工名称；空白表示官方英文回退。写入前逐项再次核验官方名称，若已恢复则官方名称优先，其他情况才按订阅归属受限地保存人工中文或英文回退。匿名、无效或重复输入返回安全中文摘要。 |
