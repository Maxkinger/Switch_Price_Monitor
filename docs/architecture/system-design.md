...
5. 保存前 Worker 重新验证每区官方商品：US、MX、BR、HK 重新解析本区官方商品页；JP 以官方软件搜索 API 重新取得精确标题 ID、标题、发行商和类型，再以官方价格 API 确认 JP/JPY/在售状态，避免 My Nintendo Store 动态或排队页使有效候选被误拒。`automatic` 保持严格身份或已确认的高置信度本地化身份校验，`manual_selection` 与 `manual_link` 可以接受本地化名称但必须为同类型的本区官方商品。任一地区无效即整体拒绝，只有全部确认或显式跳过后才以一个 D1 批次保存地区商品与监控区域。
