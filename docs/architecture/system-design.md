...
4. 日报调度器按管理员保存的时区与播报时间汇总全部订阅；格式化层已将每款商品的当前价格、全监控区历史最低价及各地区历史最低当地货币价格转为简体中文，并在超长时按 Telegram 限制分页。发送边界按页顺序投递，且只回传安全状态码、不保存 Telegram 原始错误或凭据；Worker Secret 配置与调度接入仍待完成。
