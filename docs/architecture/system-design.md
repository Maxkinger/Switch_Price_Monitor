...
2. Worker 搜索该区候选商品并返回名称、发行商、类型、封面、官方链接及可验证的本区官方价格 ID 等信息供确认；价格 ID 不由管理员手填。
3. 管理员确认商品后，匹配器根据标题、发行商、商品类型和关联信息查询其他启用地区，并为每区独立取得与验证价格 ID。
4. 低置信度或无结果地区进入人工修正：候选选择或官方链接粘贴。能确认商品而不能确认官方价格 ID 的地区仍可继续，但不得伪称官方可用。
5. 保存前生成逐区来源预览：官方价格可用、官方价格 ID 未确认且将使用的第三方顺序，或无可用价格来源。管理员确认预览后，才保存地区商品与监控区域并进入下一轮采集。
