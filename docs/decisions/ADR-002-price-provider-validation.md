...
| 任天堂日区 | [日区 Nintendo Switch 2 Edition 页面](https://store-jp.nintendo.com/item/software/D70010000106252/)可确认 Team17、Nintendo Switch 2 与升级通行证入口；该入口指向 `D70050000064985`。公开软件搜索 API 还返回美食家版组合商品 `DL_DLC` ID `70070000010202`，其公开价格接口响应为 JP/JPY、常规 ¥4,900、促销 ¥1,225。英文美食家版查询只返回同系列日文本体，使用其唯一官方片假名别名检索才返回组合商品。 | 静态 HTML 解析器不适用，但公开官方价格接口可作为专用适配器。仅已验证的 `BEE_DL`/`HAC_DL` 下载软件和 `DL_DLC` 组合商品可构造日区 Store URL；`DL_DLC` 固定映射为 `bundle`，实体和未知形态仍拒绝。没有同类型结果时允许从唯一、同发行商及同拉丁系列标记的官方日文结果派生一个别名检索一次；最终确认使用候选官方日区标题并仍要求精确 URL 与价格 ID。接口不返回完整商品身份，必须仅用于添加阶段已确认标题、发行商、类型及本区价格 ID 的地区商品；不得把 URL 的 `D` 前缀转换规则当作所有地区的通用规则。 |
