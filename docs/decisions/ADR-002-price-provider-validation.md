...
| 任天堂港区 | [任天堂香港产品公告](https://www.nintendo.com/hk/topics/article/5Jv6XsFzdrMn9E1AikxJJc)确认该作品提供 Nintendo Switch 2 Edition／升級通行證；公开检索仅得到旧 DLC 的 `ec.nintendo.com/HK` 页面，未找到升级包自身包含 HKD 和可校验身份字段的响应。 | HK 官方 JSON-LD 适配器已注册，但此样本尚未通过生产实测价格准入；不能从公告或同一作品 DLC 页面推测港币价格。当前安全结果是过期/不可用，等待升级包公开地区商品页或经审核的新官方接口。 |
...
6. 五区官方提供方注册表已实现：US、MX、BR、HK 仅尝试本区官方页面的 JSON-LD，JP 先尝试本区官方价格 API，再尝试官方页面 JSON-LD；任何地区未取得可验证价格都会安全产生过期状态，而不是推测金额。日区 API 已实现并对地区、币种和本区价格 ID 二次校验；各区公开商店的实际可用性仍以本 ADR 的逐区受控验收为准。
