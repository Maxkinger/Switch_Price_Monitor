...
| 项目 | 内容 |
| --- | --- |
| 状态 | 已确认；当前 Node 五区官方采集链路已实现，第三方实际回退待来源许可 |
| 日期 | 2026-07-17 |
| 决策 | 任天堂官方优先。eShop Prices、NT Deals、Deku Deals、Green Pipe 可在设置中列为候选来源，但在逐站取得 API 或书面许可、完成覆盖与条款审查前，生产代码不创建第三方提供方，也不发起第三方网络请求。 |

## 背景

本项目需要监控美国、日本、墨西哥、巴西、香港五区的完整游戏、DLC 和升级包。价格来源不能只“能搜索到页面”，还必须能在服务端稳定取得价格、原始货币、标题、发行商和商品类型，并能在站点变化时安全回退。
...
下表保留 2026-07-16 至 2026-07-19 的 Cloudflare Worker/Browser Run 观察证据，用于说明来源准入为什么成立或被拒绝；它不是当前平台合同。当前仓库只支持 Node.js 22、PostgreSQL 17 与本地 Playwright Chromium，旧 Worker、D1 和 Browser Binding 代码已经移除。来源的商品身份、地区、币种、官方价格 ID、超时、脱敏和失败回退规则保持不变。

日区浏览器关系发现现由 app 容器中的本地 Chromium 完成，不再调用 Cloudflare Browser Run。一次批次最多一个浏览器、三个隔离上下文串行、单项 30 秒且不自动重试；浏览器只用于订阅候选与保存前复核，不参与价格采集。

## 已实现的技术边界

1. `ProviderChain` 只在 Node 服务端按管理员设置的顺序调用来源；浏览器不直接访问任何商店或第三方站点。
...
- 在隔离 PostgreSQL 测试环境执行受控采集，确认身份校验失败会回滚且不会写入快照，官方失败时保留上一条成功快照并标记过期；第三方在许可前应明确显示“未接入”，而非出现伪造价格。
- 若要接入任一第三方站点，先记录其许可或 API 文档、允许的请求方式和频率、五区覆盖、原始本币字段与商品身份字段；完成审查后另行更新本 ADR、实现和测试，不能仅修改设置开关。
- 如果任一新来源需要浏览器自动化或认证 Cookie，先更新 ADR 并单独评估安全、资源和许可，不把它隐式加入生产采集。当前本地 Playwright 只获准处理日区升级包官方关系，不能据此扩展到第三方价格站。
