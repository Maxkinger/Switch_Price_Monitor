...
| 开发期全局免认证 | 状态接口固定可进入；缺失与伪造 Cookie 都通过共享守卫，且守卫不得读取会话服务。业务输入校验、参数化 SQL、事务与秘密排除仍必须通过；本状态只能本机开发，不能部署。 |
...
### 3.24 开发阶段全局免管理员认证回归（2026-08-10）

- 新 Node/PostgreSQL 实现把 `GET /api/auth/status` 固定为 `{ initialized: true, authenticated: true }`，共享 `requireAdmin` 不读取 Cookie 或会话服务；缺失与伪造 Cookie 均只能触发本机开发直入，不能被持久化为认证资料。
- 测试先行先确认旧守卫仍调用会话服务而失败，再以最小实现转绿。完整 Vitest 在 Compose tmpfs 的受限 `switch_test` 数据库中通过 69 个文件、420 项测试；DOM 16 项、TypeScript 严格检查和 Node 生产构建也通过。
- 订阅、详情、商品发现、来源预览等无 Cookie 路径仍验证输入白名单、官方来源约束、参数化 SQL、显式事务和秘密字段排除。该回归不构成 Docker、NAS、局域网或公网部署许可；认证恢复必须独立设计并重新验收匿名拒绝、登录、锁定、恢复码与会话撤销。

### 3.25 无认证代理设置与迁移回归（2026-08-10）

- 当前 `main` 新增 `0003_proxy_settings.sql`，在目标价永久删除迁移之后仅扩展 `settings` 的启用开关、协议、主机和端口；不恢复目标价表、列、事件或页面控件。迁移、仓储、API、浏览器与备份恢复测试均使用合成回环地址和固定测试值，不记录真实代理、密码或令牌。
- 统一出站层为任天堂、汇率、Telegram 与 Playwright 固化每次业务操作的代理快照。读取型请求只在代理传输失败时直连一次；Telegram 在发送结果不明时不重发；浏览器在回退前顺序关闭 page、context 和 browser。设置页连接测试不保存草稿，只返回两个固定 HTTPS 通道的安全三态。
- 本机 `LOCAL_DEVELOPMENT_AUTH_BYPASS=true` 继续只影响会话判定，代理端点校验、参数化 SQL、代理失败回退和秘密排除没有放宽。该实现不授权 Docker、NAS、局域网或公网部署。
