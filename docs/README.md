...
| [NAS Docker 与 PostgreSQL 迁移实施计划](superpowers/plans/2026-07-27-nas-docker-postgresql-migration.md) | 平台中立重构、PostgreSQL、Node、调度器、本地 Playwright、容器、备份、发布和 NAS 切换的测试先行步骤 | 实施中；Task 1–7 已完成本地实现与验证，Task 5 使用原生 Node Fetch 适配器，Hono 依赖待环境允许后评估 |
| [网络代理设置设计规格](superpowers/specs/2026-08-01-network-proxy-settings-design.md) | NAS Node 全部外部请求的无认证 HTTP/HTTPS/SOCKS5 代理、直连回退与连接测试 | 已实施本地代码与门禁；DS423+ 真机验收待授权 |
| [网络代理设置实施计划](superpowers/plans/2026-08-01-network-proxy-settings.md) | PostgreSQL 代理字段、统一出站层、全部提供方与 Playwright 接线、设置 API/UI 及 M1/NAS 验收 | Task 1–7 本地实现与回归完成；最终 NAS 验收待授权 |
