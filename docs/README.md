...
| [GitHub Actions PostgreSQL 双角色初始化修复设计规格](superpowers/specs/2026-08-01-github-actions-postgres-dual-role-design.md) | 让普通 CI 与发布门禁复用生产初始化脚本，以独立管理角色创建最小权限应用角色 | 已完成；普通 CI 的数据库、Gitleaks 与双架构构建远程门禁全部通过 |
| [GitHub Actions PostgreSQL 双角色初始化修复实施计划](superpowers/plans/2026-08-01-github-actions-postgres-dual-role.md) | 双角色 CI 合同、初始化与权限自检的测试先行执行步骤 | 已完成；远程 run `30685670944` 完整通过，尚未创建发布标签或推送镜像 |
