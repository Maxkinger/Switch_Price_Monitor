...
| [日区升级包 Browser Run 生产集成设计规格](superpowers/specs/2026-07-19-japanese-upgrade-browser-run-production-integration-design.md) | 单 Worker Browser Binding、同步发现、保存前二次关系验证、人工链接兜底及资源/日志边界 | 已实现；远程只读验收通过；待生产部署 |
| [日区升级包 Browser Run 生产集成实施计划](superpowers/plans/2026-07-19-japanese-upgrade-browser-run-production-integration.md) | 固定依赖、根商品、浏览器批处理、官方报价、发现/确认接线、前端重试和远程只读验收 | Task 1–8 已实现并完成只读验收；待文档提交与生产部署 |
