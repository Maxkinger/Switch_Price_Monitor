...
**Tech Stack:** TypeScript 5.8.3、Vitest 4.1.0、`@cloudflare/playwright` 1.3.0、Cloudflare Browser Run；Wrangler 4.31.0 作为失败基线，根因验证升级到与现有 Workers Types 精确兼容的 4.107.1。
...
- Modify after root-cause evidence: `/tmp/switch-price-monitor-jp-browser-probe/package-lock.json`：只把 Wrangler/Miniflare 更新到与 Playwright 1.3.0 CDP 协议兼容的当前锁定版本。
...
2026-07-19 首次本地 `launch-only` 在浏览器下载期间返回 `{ status: "failure", executionTarget: "local", stage: "browser-launch", errorName: "TypeError", errorMessage: "Cannot read properties of null (reading 'accept')", elapsedMs: 20411 }`。浏览器下载完成后，不修改代码再次执行同一请求，仍在 652 ms 内返回相同阶段和错误，故“仅首次下载竞态”假设已被否定。

完整读取已安装依赖后确认协议断层：`@cloudflare/playwright` 1.3.0 的 `connectDevtools()` 请求 `/v1/devtools/browser/{sessionId}` 并要求 WebSocket 响应；Wrangler 4.31.0 内置 Miniflare Browser Binding 只实现旧 `/v1/connectDevtools` 路径，对新路径返回普通 405 响应。Playwright 随后对空的 `response.webSocket` 调用 `accept()`，与真实错误完全一致。Cloudflare 官方变更记录也确认 Playwright 1.3.0 已改用标准 CDP。当前 npm 最新版 Wrangler 4.110.0 要求 `@cloudflare/workers-types` 5.x，与本探针锁定的 4.x 类型不兼容；安装已被 npm 的 peer dependency 校验拒绝，未使用 `--force`。官方 npm 元数据显示 Wrangler 4.107.1 精确要求 `@cloudflare/workers-types ^4.20260702.1`，与探针现有版本一致，并内置 Miniflare 4.20260702.0。

单一修复假设：只把临时探针的 Wrangler 从 4.31.0 升级到 4.107.1，使本地 Miniflare 支持 Playwright 1.3.0 使用的新 CDP 路径；不修改探针源代码、`@cloudflare/playwright`、Workers Types、兼容日期或 Browser Binding 配置。

- [x] **Step 0A：把已失败的本地请求作为集成 RED**

Evidence: `/tmp/jp-browser-launch-local.json` 与 `/tmp/jp-browser-launch-local-retry.json` 均为 `browser-launch` / `TypeError` / `accept` 失败；第二次失败发生在浏览器下载完成后，证明测试可以稳定复现协议不兼容。

- [x] **Step 0B：只升级临时 Wrangler 并更新锁文件**

Run: `npm install --save-dev --save-exact wrangler@4.107.1`

Workdir: `/tmp/switch-price-monitor-jp-browser-probe`

Expected: `package.json` 仅把 `wrangler` 改为 `4.107.1`，`package-lock.json` 更新其传递依赖；`@cloudflare/playwright` 仍为 `1.3.0`，`@cloudflare/workers-types` 仍为 `4.20260702.1`，其他顶层依赖版本不变。该变更仅存在于 `/tmp`，不进入生产仓库。

- [x] **Step 0C：升级后先运行全部纯测试和类型检查**

Run: `npm test && npm run typecheck`

Expected: 5 个测试文件、30 个测试全部通过，TypeScript exit 0；升级不得改变诊断字段或安全边界。

- [x] **Step 0D：重新启动本地会话并执行一次相同 launch-only**

Run: `npm run dev:local`

Then: `curl --fail-with-body --silent --show-error --request POST --output /tmp/jp-browser-launch-local-fixed.json http://127.0.0.1:8791/diagnostic/launch`

Expected: `{ status: "success", executionTarget: "local", stage: "complete", elapsedMs: <non-negative integer> }`。成功即确认版本协议断层是根因；若仍失败，停止并形成新的单一假设，不继续远程诊断。

首次使用 Wrangler 4.107.1 启动时，旧协议 `accept` 错误不再出现，但 Worker 在执行代码前被运行时明确拒绝：配置日期 `2026-07-18` 晚于该版本内置 workerd 支持的最新日期 `2026-07-09`。Playwright 1.3.0 的标准 CDP 路径只要求兼容日期不早于 `2026-03-17`，Cloudflare 当前官方入门示例也使用 `2026-07-09`。因此该日期差异是可独立验证的配置边界，不涉及探针逻辑。

- [x] **Step 0E：把临时兼容日期收敛到运行时支持的官方示例日期**

Modify: `/tmp/switch-price-monitor-jp-browser-probe/wrangler.jsonc`

把 `compatibility_date` 从 `2026-07-18` 改为 `2026-07-09`，并用中文注释说明：该日期仍晚于 Playwright 1.3.0 标准 CDP 的 `2026-03-17` 门槛，同时是 Wrangler 4.107.1 内置 workerd 明确支持的上限；不得为了追随日历日期写入运行时尚不支持的值。

- [x] **Step 0F：重复全量测试并再次执行同一本地 GREEN 验证**

Run: `npm test && npm run typecheck`

Then: `npm run dev:local`，并再次调用固定 `POST /diagnostic/launch`。

Expected: Worker 成功启动，诊断返回 `status: "success"`；若仍失败，停止并记录新的阶段证据，不执行远程诊断。

- [x] **Step 1：启动本地浏览器诊断会话**
...
- [x] **Step 2：执行一次本地 launch-only 并校验白名单**
...
#### 已取得的任天堂单次 RED 证据与 URL 根因

远程启动和固定普通页导航均成功后，首次任天堂单次验证返回 `{ status: "invalid-official-url", elapsedMs: 6550 }`。任天堂官方详情页公开的升级包入口实际为 `https://store-jp.nintendo.com/item/software/D70050000064985`，不含末尾斜杠；现有纯函数只接受 `^/item/software/D[0-9]+/$`，因此把正确官方商品链接误判为非法。

已确认的最小修复只把路径规则调整为 `^/item/software/(D[0-9]+)/?$`，同时把两种输入统一规范化为 `https://store-jp.nintendo.com/item/software/<ID>/`。HTTPS、精确主机、空端口、空凭据、纯数字商品 ID、唯一性和可见文字规则均保持不变。

- [x] **Step 0A：新增不带末尾斜杠的失败测试**

Modify: `/tmp/switch-price-monitor-jp-browser-probe/test/probe-domain.test.ts`

新增输入 `https://store-jp.nintendo.com/item/software/D70050000064985`，期望返回带末尾斜杠的规范 URL 和原公开文字。测试注释必须说明：只兼容任天堂实际存在的尾部斜杠差异，不放宽主机、协议、商品 ID 或路径层级。

- [x] **Step 0B：运行测试并确认 RED 原因正确**

Run: `npm test -- test/probe-domain.test.ts`

Expected: FAIL，实际状态为 `invalid-official-url`，证明失败由现有尾部斜杠正则造成。

- [x] **Step 0C：最小修改 URL 捕获与规范化逻辑**

Modify: `/tmp/switch-price-monitor-jp-browser-probe/src/probe-domain.ts`

使用捕获组 `^/item/software/(D[0-9]+)/?$` 校验并提取商品 ID；返回值不得复用原始 pathname，而必须从固定官方主机、固定 `/item/software/` 层级、捕获的纯数字 ID 和固定末尾斜杠重新构造。

- [x] **Step 0D：运行目标测试、全量测试和类型检查**

Run: `npm test -- test/probe-domain.test.ts && npm test && npm run typecheck`

Expected: 目标测试通过；临时探针全部测试通过；TypeScript exit 0。原有外部主机、HTTP、自定义端口和错误路径拒绝用例必须继续通过。

- [x] **Step 1：只有 Task 4 三项均成功时，执行一次任天堂单次验证**
