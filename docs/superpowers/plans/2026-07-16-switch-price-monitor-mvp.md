...
实施状态（2026-07-16）：已完成受管理员会话保护的订阅创建、软停用与重新启用接口。创建接口校验非空且去重的地区商品列表，并验证其属于所选游戏且仍启用；重复 `gameId` 返回既有订阅，保留原有地区配置。停用仅更新 `enabled`，不删除地区配置或历史。设置、仪表盘、历史、导出及地区/目标价编辑接口仍待实现。
