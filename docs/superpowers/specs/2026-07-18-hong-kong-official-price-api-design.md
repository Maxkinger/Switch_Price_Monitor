...
已部署并完成生产受控验收。确认、本地质量门禁与生产验证日期：2026-07-18。
...
5. 响应必须同时满足：`country` 为目标地区、存在与请求 ID 完全相同的 `title_id`、`sales_status` 为 `onsale`、价格币种与地区货币一致、金额为非负安全整数。`raw_value` 的口径按受控币种标准化：JPY 为整日元，原样写入；HKD 为整港元，必须乘以 100 转为快照所需的分。任一条件不符即安全失败并允许既有官方页面/第三方回退链继续处理。
...
已确认的 HK 官方商品链接
  ...
  -> 请求固定 HK 官方价格 API
  -> 校验地区、ID、在售状态、HKD 与金额，并将整港元转换为分
...
- HK 固定夹具生成 `country=HK&lang=zh` 请求；促销响应取 `discount_price.raw_value`，无促销响应取 `regular_price.raw_value`，并将整港元正确转换为分。
...
- 生产 V0.0.5 已通过管理员授权的立即刷新验收：五区均成功，HK 显示官方 HK$140.00（约 ¥120.83），证明整港元到分的标准化、已保存价格 ID 和快照读取链路一致。
